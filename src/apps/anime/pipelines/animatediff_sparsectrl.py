...
class AnimateDiffSparseControlNetPipeline(
    ...
    def send_to_temp_device(self, device_main, 
                                  device_temp = torch.device('cpu')):
        
        self.device_main = device_main
        self.device_temp = device_temp

        self.unet.to(self.device_temp)
        self.vae.to(self.device_temp)
        self.controlnet.to(self.device_temp)
        self.text_encoder.to(self.device_temp)
        self.motion_adapter.to(self.device_temp)
        
        if self.image_encoder is not None:
            self.image_encoder.to(self.device_temp)

        # 'CLIPFeatureExtractor' object has no attribute 'to'
        # if self.feature_extractor is not None:
        #     self.feature_extractor.to(self.device_temp)
    ...
    def encode_prompt(
        ...
        """
        # send module to main device
        device = self.device_main
        ...
        # send module back to temp device
        self.text_encoder.to(self.device_temp)
    ...
    def __call__(
        ...
        self.send_to_temp_device(device_main=self._execution_device)
        device = self.device_main
        ...
        for free_init_iter in range(num_free_init_iters):
            ...
            with self.progress_bar(total=self._num_timesteps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    # controlnet_prompt_embeds = controlnet_prompt_embeds.to(self.device_temp)
                    # controlnet_cond_mask = controlnet_cond_mask.to(self.device_temp)
                    # controlnet_cond = controlnet_cond.to(self.device_temp)
                    ...
                    self.controlnet = self.controlnet.to(self.device_temp)                 
                    self.unet = self.unet.to(device)

                    # prompt_embeds = prompt_embeds.to(self.device_temp)
                    # latent_model_input = latent_model_input.to(self.device_temp)
                    # down_block_res_samples = tuple([b.to(self.device_temp) for b in down_block_res_samples])
                    # mid_block_res_sample = tuple([b.to(self.device_temp) for b in mid_block_res_sample])
                    ...
                    self.unet = self.unet.to(self.device_temp)
